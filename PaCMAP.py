...
pacmap_res = pacmap.PaCMAP(n_components=2, n_neighbors=None, MN_ratio=1, FP_ratio=10) 
...
for i in range(len(df)):
    if df.loc[i, 'coupe'][0]=='p':
        x.append(df.loc[i, 'pacmap-2d-one'])
        y.append(df.loc[i, 'pacmap-2d-two'])
...
for i in range(len(coupe)):
    res[coupe[i]] = [pacmap_results[i,0], pacmap_results[i,1]]
