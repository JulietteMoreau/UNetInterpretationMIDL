...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb  5 08:14:05 2024
...
for i in range(len(df)):
    if df.loc[i, 'coupe'][0]=='p':
        x.append(df.loc[i, 'pca-2d-one'])
        y.append(df.loc[i, 'pca-2d-two'])
...
for i in range(len(coupe)):
    res[coupe[i]] = [pca_results[i,0], pca_results[i,1]]
